  ...
  return (
    <div className="min-h-screen p-6 max-w-xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <a
          href="/"
          className="text-sm text-muted-foreground hover:text-foreground transition cursor-pointer px-3 py-2 rounded-md hover:bg-muted/50 -mr-3"
        >
      ...
      <div className="bg-card border border-border rounded-lg p-5 mb-6">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className={`w-10 h-5 rounded-full transition relative cursor-pointer disabled:opacity-30 disabled:cursor-not-allowed ${
              config.engineerEnabled ? "bg-accent-green" : "bg-border"
            }`}
          >
            <span
              className={`absolute left-0.5 top-0.5 w-4 h-4 rounded-full bg-foreground transition-transform ${
                config.engineerEnabled ? "translate-x-5" : "translate-x-0"
              }`}
            />
          </button>
        </div>
          ...
          <div className="mt-6 space-y-8">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground/60 mb-2">
                ...
                <a
                  ...
                  className="underline hover:text-muted-foreground/80 cursor-pointer"
                >
                  aistudio.google.com
                </a>
              </p>
              ...
              {config.hasApiKey ? (
                <div className="flex items-center justify-between bg-muted rounded-md px-3 py-2.5">
                  ...
                  <button
                    type="button"
                    onClick={deleteKey}
                    className="text-xs text-accent-red/70 hover:text-accent-red transition cursor-pointer px-3 py-1.5 rounded hover:bg-accent-red/10"
                  >
                    Delete
                  </button>
                </div>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-muted border border-border rounded-md px-3 py-2.5 text-xs text-foreground placeholder:text-muted-foreground/60 focus:outline-none focus:border-accent-blue"
                  />
                  <button
                    type="button"
                    onClick={saveKey}
                    disabled={saving || !apiKey.trim()}
                    className="px-5 py-2.5 rounded-md text-xs font-medium bg-accent-blue/20 text-accent-blue hover:bg-accent-blue/30 transition cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {saving ? "Saving..." : "Save"}
                  </button>
                </div>
              ...
              {message && (
                <div className="mt-3 rounded-md px-3 py-2.5 text-xs bg-accent-red/10 text-accent-red border border-accent-red/20">
                  {message}
                </div>
              )}
            </div>
              ...
              <div className="space-y-1">
                {PERSONALITIES.map((p) => (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2.5 rounded-md text-xs transition cursor-pointer ${
                      personalityId === p.id
                  ...
                  >
                    <span className="font-medium">
                      ...
                      {personalityId === p.id ? " " : ""}{p.name}
                    </span>
                    <span className="text-muted-foreground/80 ml-1.5">— {p.description}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            {selectedPersonality.systemPrompt && (
              <div>
                ...
                <textarea
                  ...
                  className="w-full bg-muted border border-border rounded-md px-3 py-2.5 text-xs text-muted-foreground font-mono leading-relaxed resize-none focus:outline-none cursor-default"
                  rows={6}
                />
              </div>
            )}
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full bg-muted border border-border rounded-md px-3 py-2.5 text-xs text-foreground placeholder:text-muted-foreground/45 focus:outline-none focus:border-accent-blue resize-none"
                rows={6}
              />
              <p
                className={`text-xs text-accent-green/70 mt-1.5 h-4 transition-opacity ${showSaved ? "opacity-100" : "opacity-0"}`}
              >
                Saved
              </p>
            </div>
